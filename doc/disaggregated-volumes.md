...
User creates/updates DorisDisaggregatedCluster
  ...
  (includes all PVCs for naming pattern)
             ↓
 preparePersistentVolumeClaims()
   ...
   For each volume claim template:
      ...
      │   └─ If not exists: Create new PVC
      └─ All PVCs for this volume ready
             ↓
    All PVCs ready (created/updated)
             ↓
    StatefulSet Controller
      ├─ Try to create PVC → AlreadyExists
      └─ Use existing operator-created PVC
             ↓
...
The operator builds the StatefulSet first to determine the volume claim templates, then creates PVCs accordingly:

```go
func (c *Controller) preparePersistentVolumeClaims(ctx context.Context, ddc *v1.DorisDisaggregatedCluster, statefulSet *appv1.StatefulSet) bool {
    ...
    pvcList := listPVCs(ddc.Namespace, selector)
    ...
    for _, volumeName := range volumeNames {
        ...
        // 5. For each replica ordinal (0 to replicas-1)
        for ordinal := 0; ordinal < replicas; ordinal++ {
            pvcName := BuildPVCName(statefulSet.Name, ordinal, volumeName)

            if existingPVC := findPVC(pvcName, pvcMap[volumeName]) {
                ...
                createPVC(pvcName, volumeSpec, ownerReference)
            }
        }
    }
...
Operator-managed PVCs include annotations for tracking:
...
PVCs have owner references pointing to the DorisDisaggregatedCluster:
...
This ensures PVCs are cleaned up when the cluster is deleted.

## Data Safety

### Guarantees

1. **Existing PVCs are never deleted** during migration to operator management
2. **PVC names match StatefulSet pattern** exactly - ensures binding to same PVs
3. **Operator detects existing PVCs** before creating new ones
4. **Idempotent operations** - safe to run reconciliation multiple times

### Migration Scenario

**Initial state**: Cluster deployed without this feature
...
4. Adds annotations and owner references
5. Does NOT create new PVCs (they exist)
6. Continues using the same PVCs
...
|--------|---------------|---------------------|
| PVC Management | ✅ Operator-managed | ✅ Operator-managed |
...
- ✅ **Dual PVC modes** - StatefulSet-managed PVCs vs pod template PVCs
- ✅ **Intelligent routing** - Automatic routing based on volume requirements
- ✅ **Operator-managed PVCs** - Full lifecycle management for volumeClaimTemplate PVCs
- ✅ **Flexible configuration** - Mix any volume types in the same cluster
- ✅ **Backward compatible** - Existing PVC configurations work unchanged
- ✅ **Production ready** - Industry-standard Kubernetes volume management

Disaggregated Doris clusters provide complete Kubernetes volume management with intelligent routing between StatefulSet volumeClaimTemplates and pod template volumes.</content>
